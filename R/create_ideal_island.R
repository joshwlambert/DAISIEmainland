...
#' \itemize{
#'   \item{[1]: \code{branching_times}, a sorted numeric vector, as required
...
#'   \item{[2]: \code{stac}, status of colonist. In this function it can be
...
#'     colonising lineage.}
#'   \item{[3]: \code{missing_species}, a numeric value with the number of
...
create_ideal_island <- function(total_time,
  ...
  if (number_colonisations > 1) {
    ...
    col_times <- sort(
      unique(as.numeric(island_spec[, "col_t_bp"])),
      decreasing = TRUE
    )
    ...
    uniquecol <- sort(as.numeric(
      unique(island_spec[, "col_t_bp"])), decreasing = TRUE
    )
    for (i in seq_along(uniquecol)) {
      ...
      samecolonisation <- which(as.numeric(
        island_spec[, "col_t_bp"]) == uniquecol[i]
      )
