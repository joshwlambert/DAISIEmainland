...
ext_event <- function(island_tbl) {
  ...
  if (typeofspecies == "C") {
    ...
    sisters <- intersect(which(island_tbl[, "main_anc_id"] ==
    ...
    if (length(sisters) >= 3) {
      ...
      mostrecentspl <- substring(island_tbl[extinct, "branch_code"],
                                 numberofsplits)
      ...
      motiftofind <-
        paste0(substring(island_tbl[extinct, "branch_code"],
                         1,
                         numberofsplits - 1),
               sistermostrecentspl)
      possiblesister <-
        survivors[which(substring(island_tbl[survivors, "branch_code"],
                                  1,
      ...
      island_tbl[possiblesister, "branch_code"] <-
        paste0(substring(island_tbl[possiblesister, "branch_code"],
                         1,
                         numberofsplits - 1),
               substring(island_tbl[possiblesister, "branch_code"],
                         numberofsplits + 1,
