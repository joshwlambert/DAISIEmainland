test_that("ext_event produces correct output for single species", {
  ...
  expected_island_tbl <- data.frame(spec_id = numeric(),
                                     main_anc_id = numeric(),
                                     col_t = numeric(),
                                     spec_type = character(),
...
test_that("ext_event produces correct output for more than one species", {
  set.seed(
    ...
    normal.kind = "Inversion",
    sample.kind = "Rejection")
  island_tbl <- data.frame(spec_id = c(1, 2),
                            main_anc_id = c(1, 1),
                            col_t = c(0.5, 0.5),
                            spec_type = c("C", "C"),
                            branch_code = c("A", "B"),
...
test_that("ext_event produces correct output for more than two species", {
  set.seed(
    ...
    normal.kind = "Inversion",
    sample.kind = "Rejection")
  island_tbl <- data.frame(spec_id = c(1, 2, 3, 4),
  ...
  expect_true(is.data.frame(island_tbl))
  expected_island_tbl <- data.frame(spec_id = c(2, 3, 4),
                                     main_anc_id = c(1, 1, 1),
                                     col_t = c(0.5, 0.5, 0.5),
                                     spec_type = c("C", "C", "C"),
